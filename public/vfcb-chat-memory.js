...
Or you can reply with 1, 2 or 3.`;
  ...
  function jawaLegQuestion(capeType) {
    let intro = "Good — now we’ll use the figure markings.";

    if (capeType === "none") {
      intro = `Right — so your Jawa is missing its cape, what collectors would usually call a naked figure.

That’s common, so we identify it from the figure itself.`;
    }

    if (capeType === "cloth") {
      intro = "Good — so we’re dealing with a cloth cloak Jawa.";
    }

    if (capeType === "vinyl") {
      intro = "Good — that means you may have the early vinyl cape version. We need to verify the figure carefully.";
    }

    return `${intro}

Look at the back of the left leg.

Does your Jawa show:

1. © G.M.F.G.I. 1977 on top, with HONG KONG underneath
2. Just © G.M.F.G.I. 1977, with no HONG KONG underneath
3. I can’t tell / it doesn’t appear to have either
